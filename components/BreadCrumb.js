...
export default function BreadCrumb({ staticText, secondText, thirdText, forthText, color }) {
  ...
  const crumbs = router?.asPath?.split("/").filter((crumb) => crumb !== '').map((crumb, index) => {
    ...
    return (
      <div key={crumb} className={`mr-5  flex items-center hover:text-lava whitespace-nowrap ${currentLink === router.asPath ? "pointer-events-none underline underline-offset-8	ml-[-10px]" : ""}`}>
