    ...
    return (
        <div className={show ? `sidedrawer transform translate-x-0 fixed top-0 right-0 w-full z-[-1] transition-all bg-[#1E1E1E] duration-500 ease-in-out` : ` fixed top-0 right-0 w-0  z-[5] transform transition-all shadow-lg ease-in-out `}>
            <div className="container xl:flex h-screen pt-[100px] lg:pt-0">
                <div className="m-auto h-full grid grid-cols-1 xl:grid-cols-2 xl:gap-10 items-center">
                            ...
                            menuData?.map((menuItem,index ) => {
                                return (
                                    <Link key={menuItem.id} href={menuItem.link}>
                                        <li className="cursor-pointer hover:text-white transition-all ease-in-out first:hover:text-blue">{menuItem.title}</li>
                                    </Link>
                                )
                            })
                        }
                    ...
                    <ul className="hidden xl:block text-[#C2C5C9] text-center leading-[30px] text-[20px] border border-border px-[80px] py-[48px] max-w-[504px] h-fit">
                        <li>Terms & Conditions</li>
                        <li className="mt-[22px] ">Privacy Policy</li>
                        <li className="mt-[22px]">Contact Us</li>
                        <div className="flex items-center justify-center mt-[32px]">
